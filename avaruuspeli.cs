...
public class avaruuspeli : PhysicsGame
{
   ...
   private void AddEnemies()
    ...
    {
        PhysicsObject power = new PhysicsObject(2 * 10, 2 * 10);
        power.Shape = Shape.Diamond;
        power.Color = Color.Aqua;
        Add(power);
        power.Tag = "powerup";
    }
    ...
    private IntMeter CreatePointCounter(string counter)
    {
        ...
        pointscreen2.X = Screen.Bottom + 300;
        pointscreen2.Y = Screen.Left - 600;
    ...
    {       
        ClearAll();
        pointcounter = CreatePointCounter("Ufoja tuhottu");
        starcounter = CreatePointCounter("tähtiä");
        ...
        Timer.CreateAndStart(3, AddPowerUp);
        ...
        for (int i = 0; i < tahtia; i++)
            RandomStar(this, alaosa, 50, "tahti");
        ...
        Keyboard.Listen(Key.Up, ButtonState.Pressed, MovePlayer, "Pelaaja ylös", player, new Vector(0, 200));
        Keyboard.Listen(Key.Down, ButtonState.Pressed, MovePlayer, "Pelaaja alas", player, new Vector(0, -200));
        Keyboard.Listen(Key.Left, ButtonState.Pressed, MovePlayer, "Pelaaja vasemalle", player, new Vector(-200, 0));
        Keyboard.Listen(Key.Right, ButtonState.Pressed, MovePlayer, "Pelaaja oikealle", player, new Vector(200, 0));
